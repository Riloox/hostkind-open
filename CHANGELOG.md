...
- Terms of Use, in English and Spanish. Every user is asked to accept them on first sign-in (once per install when sign-in is turned off) before using the panel, and again whenever they change. Acceptance is stored with the account and recorded in the audit log. Settings has a new Legal section to reread them, and `TERMS.md` carries the same text in the repository.
