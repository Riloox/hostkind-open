...
async function main() {
  ...
  try {
    ...
    assert.ok(
      csp['img-src']?.some((source) => source === 'https://cdn.modrinth.com'),
      `img-src must allow Modrinth project icons; got: ${csp['img-src']?.join(' ') || '<missing>'}`
    );
  } finally {
  ...
  console.log('PASS  modrinth-icons');
}
