...
tests.push({ name: 'buildTrackerUrl builds the issue chooser URL', fn: () => {
  assert.strictEqual(buildTrackerUrl('Riloox', 'fleetdeck-open'), 'https://github.com/Riloox/fleetdeck-open/issues/new/choose');
...
tests.push({ name: 'buildTrackerUrl fails closed on bad input', fn: () => {
  ...
  assert.strictEqual(buildTrackerUrl('', 'fleetdeck-open'), null);
