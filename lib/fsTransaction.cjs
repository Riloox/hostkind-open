...
class Transaction {
  ...
  stageCopy(srcAbs, destRel) {
    // Phase 2B: no existsSync->use TOCTOU. copyFileSync is the atomic check;
    // ENOENT maps to source_missing. Destination is claimed exclusively so a
    // concurrent stage of the same rel path fails closed instead of interleaving.
    const stagedPath = this._stagedPath(destRel);
    ensureDir(path.dirname(stagedPath));
    try {
      fs.copyFileSync(srcAbs, stagedPath, fs.constants.COPYFILE_EXCL);
    } catch (err) {
      if (err && (err.code === 'ENOENT' || err.code === 'ENOTDIR')) {
        throw new StagingError(`source missing: ${srcAbs}`, 'source_missing');
      }
      if (err && err.code === 'EEXIST') {
        // Same operation staging the same path twice: overwrite deterministically.
        try { fs.copyFileSync(srcAbs, stagedPath); }
        catch (retryErr) {
          if (retryErr && (retryErr.code === 'ENOENT' || retryErr.code === 'ENOTDIR')) {
            throw new StagingError(`source missing: ${srcAbs}`, 'source_missing');
          }
          throw retryErr;
        }
...
module.exports = { Transaction, StagingError, stagingRoot, sweep, safeTransactionPath, atomicWriteFileSync };

/*
 * Unified write-tmp + rename helper (Phase 2A dedupe).
 *
 * Replaces the 5x ad-hoc `writeFileSync(tmp); renameSync(tmp, file)` blocks in
 * palworld-settings, palworld-mods, stateStore, palworld-workshop (+ terraria
 * atomicWrite). The temp file is created with `{ flag: 'wx' }` so a colliding
 * temp name fails loudly instead of overwriting whatever landed first, then
 * atomically renamed over the target (same-filesystem rename).
 */
function atomicWriteFileSync(target, data, { encoding = null } = {}) {
  const dir = path.dirname(target);
  fs.mkdirSync(dir, { recursive: true });
  const temporary = `${target}.${crypto.randomUUID()}.tmp`;
  const options = encoding == null ? { flag: 'wx' } : { flag: 'wx', encoding };
  if (Buffer.isBuffer(data)) fs.writeFileSync(temporary, data, { flag: 'wx' });
  else if (encoding == null) fs.writeFileSync(temporary, data, { flag: 'wx' });
  else fs.writeFileSync(temporary, String(data), options);
