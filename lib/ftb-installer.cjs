...
// Re-assert containment inline before any fs call (CodeQL js/path-injection
// does not model custom helper-call sanitizers, so the guard must dominate
// the sink in the same function body).
