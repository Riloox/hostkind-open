...
function stagedUploadPath(file) {
  ...
  // The containment check CodeQL recognizes (resolve + startsWith(root +
  // path.sep) with the use in the guarded branch) runs first, then the
  // direct-child shape re-assertion (js/path-injection).
...
function router(deps) {
  ...
  // (canonical helper: lib/palworld-operations.cjs), enforced as express
  // middleware ahead of the handler. Authorization is untouched: the
  // capability checks below still run.
  const limitCurate = rateLimit({ windowMs: 60_000, limit: 10, standardHeaders: false, legacyHeaders: false, keyGenerator: (req) => `curate:${req.user && req.user.id}`, handler: (req, res) => tooMany(res, req) });
  ...
  const limitInstantiate = rateLimit({ windowMs: 60_000, limit: 10, standardHeaders: false, legacyHeaders: false, keyGenerator: (req) => `instantiate:${req.user && req.user.id}`, handler: (req, res) => tooMany(res, req) });
  const limitRecover = rateLimit({ windowMs: 60_000, limit: 10, standardHeaders: false, legacyHeaders: false, keyGenerator: (req) => `recover:${req.user && req.user.id}`, handler: (req, res) => tooMany(res, req) });
  const limitDelete = rateLimit({ windowMs: 60_000, limit: 10, standardHeaders: false, legacyHeaders: false, keyGenerator: (req) => `delete:${req.user && req.user.id}`, handler: (req, res) => tooMany(res, req) });
  const limitExport = rateLimit({ windowMs: 60_000, limit: 10, standardHeaders: false, legacyHeaders: false, keyGenerator: (req) => `export:${req.user && req.user.id}`, handler: (req, res) => tooMany(res, req) });
  ...
  r.post('/', limitCurate, async (req, res) => {
    try {
      const server = deps.findServer(String(req.body?.serverId || ''));
      if (!server) return res.status(404).json({ error: 'Server not found.' });
      if (!canCurate(req.user, server.id)) return deny(res, CAPABILITIES.CONTENT_VIEW);
  ...
  // Downloads stream a full archive off disk, so they take the same per-user
  // budget as the other expensive operations.
  r.get('/:id/export', limitExport, (req, res) => {
    try {
  ...
  r.post('/import/preview', limitImports, upload.single('file'), async (req, res) => {
  ...
  r.post('/import', limitImports, async (req, res) => {
  ...
  r.post('/:id/instantiate', limitInstantiate, (req, res) => {
    const row = templates.latest(req.params.id);
    if (!row) return res.status(404).json({ error: 'Template not found.' });
   ...
   */
  r.post('/operations/:id/recover', limitRecover, (req, res) => {
  ...
  r.delete('/:id', limitDelete, (req, res) => {
    try {
      const row = templates.latest(req.params.id);
      if (!row) return res.status(404).json({ error: 'Template not found.' });
      if (!canCurate(req.user, row.source_server_id)) return deny(res, CAPABILITIES.CONTENT_VIEW);
...
function cloneRouter(deps) {
  ...
  const limitClone = rateLimit({ windowMs: 60_000, limit: 10, standardHeaders: false, legacyHeaders: false, keyGenerator: (req) => `clone:${req.user && req.user.id}`, handler: (req, res) => tooMany(res, req) });
  ...
  r.post('/:id/clone', limitClone, (req, res) => {
    const source = deps.findServer(req.params.id);
    if (!source) return res.status(404).json({ error: 'Server not found.' });
    if (!canManage(req.user, source.id)) return deny(res, CAPABILITIES.SERVER_MANAGE);
    dispatch(req, res, {
