...
const { bodyServerId } = require('../request-server.cjs');
const { CAPABILITIES, requireCap } = require('../capabilities.cjs');

module.exports = function terrariaModsRouter(deps) {
  const router = express.Router();
  const sid = (req) => (req.query && req.query.serverId) || bodyServerId(req) || deps.activeServerId();
  ...
  router.post('/workshop/install-batch', requireCap(CAPABILITIES.PLUGINS_MANAGE, scope), async (req, res) => {
    const server = serverOf(req, res);
    if (!server) return;
    try {
      const preview = await terrariaMods.downloadWorkshopBatch({
        desc: server, actorId: req.user.id, manager: deps.getManager(server.id),
        values: req.body?.values, cacheDir: deps.cacheDir(), download: deps.download,
      });
      record(req, server, 'terraria.mods.workshop.batch-preview', 'success', { count: preview.details.length });
      res.json({ ok: true, preview });
    } catch (error) { send(res, error); }
  });
  ...
  function enabledBatchHandler(enabled) {
    return (req, res) => {
      const server = serverOf(req, res);
      if (!server) return;
      const action = enabled ? 'enable' : 'disable';
      ...
      try {
        if (!req.body?.token) {
          return res.json({ ok: true, preview: terrariaMods.makeBatchPreview({
            desc: server, actorId: req.user.id, names, enabled, manager: deps.getManager(server.id),
          }) });
        }
        const result = mutate(req, res, server, `terraria-mod-batch-${action}`, { count: names.length, action }, () => (
          terrariaMods.setEnabledBatch({
            desc: server, actorId: req.user.id, token: req.body.token, manager: deps.getManager(server.id),
          })
        ));
        if (!result) return;
        record(req, server, `terraria.mods.batch.${action}`, 'success', { count: result.changed.length });
        res.status(202).json(result);
      } catch (error) {
        record(req, server, `terraria.mods.batch.${action}`, 'failure', { count: names.length, code: error.code || 'failed' });
        send(res, error);
      }
    };
  }

  router.post('/batch/enable', requireCap(CAPABILITIES.PLUGINS_MANAGE, scope), enabledBatchHandler(true));
  router.post('/batch/disable', requireCap(CAPABILITIES.PLUGINS_MANAGE, scope), enabledBatchHandler(false));
