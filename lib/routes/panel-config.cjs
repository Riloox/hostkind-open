 ...
 * requireAdmin travels with each writer route, the /fs browser, and the native
 * folder picker. GET /config is readable by any principal, so it never carries
 * server descriptors or schedules and non-admins get a short allowlist.
 *
...
module.exports = function panelConfigRouter({
  ...
  function publicConfig() {
    ...
    // Every principal can read this route, including operators scoped to one
    // server and API keys with no grants. Server descriptors carry game
    // passwords, launch arguments, and folders, and schedules name servers:
    // both are served per-server (filtered) by /api/servers and /api/tasks.
    delete c.servers;
    delete c.tasks;
    delete c.taskState;
    return c;
  ...
  // Panel-wide settings a non-admin screen may read. Anything else in the
  // config (allowed origins, integrations, backup folders) is admin business.
  const NON_ADMIN_KEYS = ['appName', 'branding', 'gameAccents', 'requireAuth', 'sessionHours', 'consoleHistoryLines', 'geoLanguageDetection', 'watchdog'];
  function configFor(user) {
    const c = publicConfig();
    if (user && user.role === 'admin') return c;
    const out = {};
    for (const key of NON_ADMIN_KEYS) if (c[key] !== undefined) out[key] = c[key];
    if (c.backups) out.backups = { maxCount: c.backups.maxCount, maxSizeMB: c.backups.maxSizeMB };
    return out;
  }

  router.get('/config', (req, res) => res.json(configFor(req.user)));
  ...
  // ---------------------------------------------------------------------------
  router.get('/pick-folder', requireAdmin, async (req, res) => {
