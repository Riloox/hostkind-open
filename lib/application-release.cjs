...
// Strict stable semver: X.Y.Z with an optional fourth numeric patch-build
// component (X.Y.Z.W), no prerelease/build suffix, no v prefix, no leading zeros.
const STRICT_SEMVER = /^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)(?:\.(0|[1-9]\d*))?$/;
...
function parseVersion(value) {
  if (typeof value !== 'string' || !STRICT_SEMVER.test(value)) {
    throw new Error(`Invalid version string (expected X.Y.Z[.W]): ${JSON.stringify(value)}`);
  }
 ...
 * Compares component-wise over 4 positions, treating a missing 4th
 * component as 0, so 0.1.3 < 0.1.3.1 < 0.1.3.2 < 0.1.4.
 * Throws on anything that is not strict stable X.Y.Z[.W].
 */
...
function validateManifest(manifest, options = {}) {
  ...
  try {
    ...
    fail(`version must be strict stable semver X.Y.Z[.W] (no prerelease/build/v prefix), got ${JSON.stringify(manifest.version)}`);
  }
...
function createReleaseClient({ fetchImpl, repository, manifestUrl, verifyManifest }) {
  ...
  async function getLatest({ platformKey, currentVersion }) {
    ...
    const normalizedCurrent = normalizeInstalledVersion(currentVersion);
    try {
      parseVersion(normalizedCurrent);
    } catch (error) {
      throw updaterError('VALIDATION_ERROR', `currentVersion must be strict stable semver X.Y.Z[.W], got ${JSON.stringify(currentVersion)}`, error);
    }
    ...
    if (compareVersions(validated.version, normalizedCurrent) <= 0) {
      return { available: false, currentVersion: normalizedCurrent, checkedAt };
    }
    return {
      available: true,
