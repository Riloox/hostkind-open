...
function setEnabled({ activeDir, disabledDir, names, enabled }) {
  ...
  try {
    for (const move of moves) {
      // Phase 2B: atomic claim with O_EXCL instead of existsSync->rename
      // TOCTOU. claimExclusiveSync throws EEXIST when the destination
      // appeared between our readdir and this move.
      claimExclusiveSync(move.to, move.name);
...
/*
 * Phase 2B TOCTOU guard: atomically claim a destination path with O_EXCL
 * (wx) so existsSync->rename races fail closed with addon_conflict instead
 * of silently overwriting a concurrent move. The claim file is removed
 * immediately; the rename that follows is the real commit.
 */
function claimExclusiveSync(destPath, name) {
  let fd = null;
  try {
    fd = fs.openSync(destPath, 'wx');
  } catch (err) {
    if (err && err.code === 'EEXIST') {
      throw new AddonStateError(`${name || destPath} already exists in the destination folder.`, 409, 'addon_conflict');
    }
    throw err;
  }
...
module.exports = { MAX_SELECTION, AddonStateError, disabledDir, list, setEnabled, claimExclusiveSync };
