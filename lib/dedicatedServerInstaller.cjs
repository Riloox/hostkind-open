...
async function ensureSteamCmd(cacheDir, download, onProgress = () => {}) {
  ...
  // Phase 2B: single-flight the SteamCMD bootstrap with an O_EXCL lock file
  // so concurrent installs don't both pass existsSync and download/extract
  // over each other. The loser waits for the winner's executable.
  const lockPath = path.join(cacheDir, 'steamcmd.lock');
  let lockFd = null;
  let owned = false;
  try {
    ...
    // Stale lock: break it and proceed.
