...
function writeMarker(destination, payload) {
  ...
  // Phase 2B: claim the destination marker with O_EXCL (wx) so two concurrent
  // beginInstall calls for the same folder fail closed instead of both
  // passing existsSync and interleaving. EEXIST means another create owns it.
  const marker = markerPath(destination);
  let fd = null;
  try {
  ...
  try { fs.writeSync(fd, JSON.stringify({ schema: 1, ...payload }, null, 2)); }
