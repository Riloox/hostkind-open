...
/*
 * Phase 2B retention for product_events (append-only analytics), mirroring
 * health.runRetention. Idempotent; keeps the horizon window.
 */
const PRODUCT_EVENTS_RETENTION_MS = 90 * 24 * 3600 * 1000;

function pruneProductEvents({ now = Date.now(), retentionMs = PRODUCT_EVENTS_RETENTION_MS } = {}) {
  const db = open();
  const floor = now - retentionMs;
  const pruned = db.prepare('DELETE FROM product_events WHERE occurred_at < ?').run(floor).changes;
  const expiredChallenges = db.prepare('DELETE FROM pairing_challenges WHERE used_at IS NOT NULL AND used_at < ?').run(floor).changes;
  return { pruned, expiredChallenges };
}
