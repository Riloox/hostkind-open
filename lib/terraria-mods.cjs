...
const { createPreviewStore } = require('./previewStore.cjs');
const { atomicWriteFileSync } = require('./fsTransaction.cjs');
const { PREVIEW_TTL_MS_DEFAULT, SNAPSHOT_RETENTION_DEFAULT } = require('../config/constants.cjs');

const INVENTORY_VERSION = 1;
const MAX_MOD_BYTES = 2 * 1024 * 1024 * 1024;
const MAX_FILES = 100000;
const PREVIEW_TTL_MS = PREVIEW_TTL_MS_DEFAULT;
const previews = createPreviewStore({
  ttlMs: PREVIEW_TTL_MS,
  onExpire: (_token, preview) => {
    try { if (preview && preview.staging) fs.rmSync(preview.staging, { recursive: true, force: true }); } catch (_) { /* swept later */ }
  },
});
...
function applyImport({ desc, actorId, manager, token, replace = false }) {
  ...
  try {
    ...
    const sources = readSources(desc);
    for (const incoming of preview.incoming) {
      sources.mods[incoming.internalName] = incoming.source || preview.source || { provider: 'local', importedAt: new Date().toISOString() };
    }
...
function workshopSelection(values) {
  if (!Array.isArray(values)) values = [values];
  const ids = [...new Set(values.map((value) => parseWorkshopId(value)))];
  if (!ids.length) fail('Select at least one Workshop item.', 400, 'selection_required');
  if (ids.length > 50) fail('Select no more than 50 Workshop items at once.', 400, 'selection_too_large');
  return ids;
}

async function downloadWorkshopBatch({
  desc, actorId, manager, values, cacheDir, download, fetchImpl,
  resolveDetail = null, ensureSteamCmdImpl = ensureSteamCmd, runSteamImpl = runSteamCmd,
  locateWorkshopContentImpl = locateWorkshopContent, selectWorkshopTmodsImpl = selectWorkshopTmods,
}) {
  requireOffline(manager);
  const ids = workshopSelection(values);
  const resolved = await mapConcurrentSettled(ids, async (id) => (
    resolveDetail ? resolveDetail(id) : resolveWorkshop(id, { fetchImpl, force: true })
  ), { concurrency: 4 });
  const failed = [];
  const details = [];
  resolved.forEach((result, index) => {
    if (result.status === 'fulfilled') details.push(result.value);
    else failed.push({ id: ids[index], error: result.reason?.message || 'Workshop lookup failed.' });
  });
  if (!details.length) throw new Error(failed[0]?.error || 'Workshop lookup failed.');
  const steamcmd = await ensureSteamCmdImpl(cacheDir, download);
  const args = ['+login', 'anonymous'];
  for (const detail of details) args.push('+workshop_download_item', WORKSHOP_APP_ID, detail.id, 'validate');
  args.push('+quit');
  try {
    await runSteamImpl(steamcmd, args, { cwd: path.dirname(steamcmd) });
  } catch (_) {
  ...
  try {
    ...
    for (const detail of details) {
      const content = locateWorkshopContentImpl(steamcmd, detail.id);
      const files = selectWorkshopTmodsImpl(content, desc, detail.id);
      ...
      for (const file of files) {
        const target = path.join(staging, `${incoming.length}-${path.basename(file)}`);
    ...
    const plan = incoming.map((mod) => {
      const installed = current.mods.find((item) => item.internalName === mod.internalName);
      return { ...mod, stagedFile: undefined, source: undefined, action: installed ? 'replace' : 'add', installedVersion: installed?.version || null };
    });
    const token = crypto.randomUUID();
    previews.set(token, {
      token, action: 'import', serverId: desc.id, actorId, fingerprint: current.fingerprint,
      staging, incoming, plan, source: details.length === 1 ? incoming[0].source : null,
      expiresAt: Date.now() + PREVIEW_TTL_MS,
    });
    return {
      token, detail: details.length === 1 ? details[0] : null, details, plan, failed,
...
async function downloadWorkshop(options) {
  const result = await downloadWorkshopBatch({ ...options, values: [options.value] });
  return { ...result, detail: result.details[0] };
}
...
function makeBatchPreview({ desc, actorId, names, enabled, manager }) {
  requireOffline(manager);
  if (!Array.isArray(names)) fail('names must be an array.', 400, 'invalid_names');
  const selectedNames = [...new Set(names.map((name) => String(name || '').trim()).filter(Boolean))];
  if (!selectedNames.length) fail('Select at least one mod.', 400, 'empty_selection');
  if (selectedNames.length > 50) fail('Select no more than 50 mods at once.', 400, 'selection_too_large');
  if (typeof enabled !== 'boolean') fail('enabled must be a boolean.', 400, 'invalid_enabled');

  const state = inventory(desc);
  const selected = selectedNames.map((name) => {
    const mod = state.mods.find((candidate) => candidate.internalName === name);
    if (!mod) fail(`Mod ${name} was not found.`, 404, 'mod_not_found');
    return mod;
  });
  if (enabled) {
    const missing = selected
      .flatMap((mod) => mod.dependencies)
      .filter((dep, index, deps) => !deps.slice(0, index).some((prior) => prior.internalName === dep.internalName))
      .filter((dep) => !state.mods.some((candidate) => candidate.internalName === dep.internalName));
    if (missing.length) fail(`Install the missing dependencies first: ${missing.map((dep) => dep.internalName).join(', ')}.`, 409, 'missing_dependencies');
  }

  const changes = selected.filter((mod) => mod.enabled !== enabled);
  const action = enabled ? 'enable' : 'disable';
  const token = crypto.randomUUID();
  const preview = {
    token, serverId: desc.id, actorId, action, batch: true, names: selectedNames,
    enabled, selected: selected.map((mod) => ({ internalName: mod.internalName, displayName: mod.displayName, file: mod.file, enabled: mod.enabled })),
    plan: {
      add: [], remove: [],
      enable: enabled ? changes.map((mod) => ({ internalName: mod.internalName, displayName: mod.displayName, file: mod.file })) : [],
      disable: enabled ? [] : changes.map((mod) => ({ internalName: mod.internalName, displayName: mod.displayName, file: mod.file })),
    },
    fingerprint: state.fingerprint, expiresAt: Date.now() + PREVIEW_TTL_MS,
    requiresOffline: true, restartRequired: changes.length > 0,
  };
  previews.set(token, preview);
  return { ...preview, actorId: undefined, fingerprint: undefined };
}

function setEnabledBatch({ desc, actorId, token, manager }) {
  requireOffline(manager);
  const preview = previews.get(String(token || ''));
  if (!preview || preview.expiresAt < Date.now()) fail('That preview expired. Review the change again.', 409, 'preview_expired');
  if (preview.actorId !== actorId || preview.serverId !== desc.id || !preview.batch || !['enable', 'disable'].includes(preview.action)) {
  ...
  const current = new Set(state.enabled);
  const next = preview.enabled
    ? [...new Set([...state.enabled, ...preview.names])]
    : state.enabled.filter((name) => !preview.names.includes(name));
  const changed = preview.names.filter((name) => current.has(name) !== preview.enabled);
  previews.delete(String(token));
  if (!changed.length) return { ok: true, changed: [], restartRequired: false };
  const snapshot = snapshotMods(desc, `Before batch ${preview.action} of ${changed.length} mods`);
  writeEnabled(desc, next);
  return { ok: true, changed, snapshotId: snapshot.id, restartRequired: true };
}
...
function snapshotMods(desc, reason) {
  ...
  const snapshot = snapshots.take({
    ...
    kind: 'terraria-mod',
    reason,
    retention: SNAPSHOT_RETENTION_DEFAULT,
  });
