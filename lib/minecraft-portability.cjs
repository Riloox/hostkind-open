...
function launchArgValue(text, key) {
  const tokens = String(text).split(/\s+/).filter(Boolean);
  const inline = tokens.find((token) => token.startsWith(`${key}=`));
...
/*
 * Forge and NeoForge installations made by their installers do not always
 * leave a runnable jar in the server root. Modern installs can instead leave
 * a platform-specific argfile under libraries/ and a run.bat/run.sh wrapper.
 * Reuse the same launch shape as the normal create flow, but only accept an
 * argfile whose path identifies the loader. This keeps an arbitrary text file
 * from becoming an importable Java command.
 */
function detectGeneratedLaunch(dir, platform = process.platform) {
  const { findForgeLaunchTarget } = require('./serverInstaller.cjs');
  for (const loader of ['neoforge', 'forge']) {
    const target = findForgeLaunchTarget(dir, loader, platform);
    if (!target?.launchArgs?.length) continue;
    const argRef = target.launchArgs.find((arg) => typeof arg === 'string' && arg.startsWith('@'));
    if (!argRef) continue;

    const normalizedRef = argRef.slice(1).toLowerCase();
    const inferredLoader = normalizedRef.includes('/neoforged/')
      ? 'neoforge'
      : normalizedRef.includes('/minecraftforge/')
        ? 'forge'
        : null;
    if (inferredLoader !== loader) continue;

    const argPath = path.resolve(dir, argRef.slice(1));
    let text;
    try { text = fs.readFileSync(argPath, 'utf8'); } catch { continue; }

    const label = loader === 'neoforge' ? 'NeoForge' : 'Forge';
    return {
      jar: { jar: target.jar, type: loader, label },
      launchArgs: target.launchArgs,
      loader,
      mcVersion: launchArgValue(text, '--fml.mcVersion'),
    };
  }
  return null;
}

function detectJarLoader(dir, platform = process.platform) {
  let entries = [];
  ...
  const generated = !detected ? detectGeneratedLaunch(dir, platform) : null;
  if (generated) detected = generated.jar;
  ...
  return {
    jar: detected,
    jars: rootJars,
    launchArgs: generated?.launchArgs || null,
    loader: generated?.loader || detected?.type || null,
    mcVersion: generated?.mcVersion || '',
    hasLibraries,
    hasVersions,
    hasMods,
    hasPlugins,
