...
/*
 * Phase 2B: prune helpers mirroring health.runRetention. Audit events are
 * append-only; automatic pruning is opt-in (manual retention flow stays the
 * source of truth) with a generous default horizon. Expired previews and
 * old idempotency records are always safe to sweep.
 */
const AUDIT_EVENTS_RETENTION_MS = 365 * 24 * 3600 * 1000;
const AUDIT_PREVIEW_TTL_MS = 10 * 60 * 1000;

function pruneAudit({ now = Date.now(), retentionMs = null } = {}) {
  const db = open();
  return db.transaction(() => {
    const expiredPreviews = db.prepare('DELETE FROM audit_retention_previews WHERE expires_at < ?').run(now).changes;
    let prunedEvents = 0;
    if (retentionMs != null) {
      prunedEvents = db.prepare('DELETE FROM audit_events WHERE ts < ?').run(now - retentionMs).changes;
    }
    // Idempotency records older than the event horizon are dead weight.
    const staleRequests = db.prepare('DELETE FROM audit_retention_requests WHERE created_at < ?').run(now - AUDIT_EVENTS_RETENTION_MS).changes;
    return { expiredPreviews, prunedEvents, staleRequests };
  })();
}

module.exports = { record, list, all, genId, safeJson, pruneAudit, AUDIT_EVENTS_RETENTION_MS, AUDIT_PREVIEW_TTL_MS };
