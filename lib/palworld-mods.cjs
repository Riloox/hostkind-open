...
const previews = createPreviewStore({
  ...
  onExpire: (_token, preview) => {
    try { if (preview && preview.archivePath && fs.existsSync(preview.archivePath)) fs.unlinkSync(preview.archivePath); } catch (_) { /* swept later */ }
  },
});
const frameworkPreviews = createPreviewStore({ ttlMs: PREVIEW_TTL_MS });
...
function writeInventory(server, inventory) {
  ...
  const file = inventoryPath(server);
  atomicWriteFileSync(file, JSON.stringify({ version: INVENTORY_VERSION, packages: inventory.packages }, null, 2));
}
