...
function createApplicationUpdater({ releaseClient, installer, stateStore, platformKey, currentVersion, now, logger = console }) {
  ...
  // A state-store write failure (e.g. a read-only install directory) must
  // never wedge the in-memory machine: persist() runs before the try blocks
  // below, so a throw here would strand the updater in "checking" forever.
  // The durable record is a recovery aid, not a precondition.
  function persist() {
    ...
    try {
      ...
      try {
        if (logger && typeof logger.warn === 'function') logger.warn('[application-updater] could not persist update state', error && error.message ? error.message : error);
      } catch { /* logging must not break the updater */ }
    }
  }
