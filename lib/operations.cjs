...
const { DESTRUCTIVE_KIND_HINTS } = require('../config/constants.cjs');
...
// (Phase 2A: hints live in config/constants.cjs; the duplicate 'world-write'
// entry is removed.)
...
function acquireServerLock(operationId, serverId) {
  ...
  const check = () => {
    // Phase 2B: both QUEUED and RUNNING claim the per-server lock. A queued
    // operation owns its staging directory and destination claim; letting a
    // second queued operation past the gate caused double-claim TOCTOU on
    // install destinations and template staging areas.
    const conflict = db.prepare(`
      SELECT id FROM operations
       WHERE server_id = ? AND state IN (?, ?)
         AND id != ?
    `).get(serverId, STATES.QUEUED, STATES.RUNNING, operationId);
    return !conflict;
  };
...
/*
 * Phase 2B retention for the append-only operation_events timeline, mirroring
 * health.runRetention: idempotent, bounded, safe to run every tick. Keeps the
 * newest N events per operation plus everything newer than the horizon, so
 * active operations never lose their timeline mid-flight.
 */
const OPERATION_EVENTS_PER_OP_KEEP = 500;
const OPERATION_EVENTS_RETENTION_MS = 30 * 24 * 3600 * 1000;

function pruneOperationEvents({ now = Date.now(), perOpKeep = OPERATION_EVENTS_PER_OP_KEEP, retentionMs = OPERATION_EVENTS_RETENTION_MS } = {}) {
  const db = open();
  const floor = now - retentionMs;
  return db.transaction(() => {
    // Drop old rows for terminal operations only; non-terminal timelines stay
    // intact so resume/recovery can replay them.
    const terminalIds = db.prepare(`SELECT id FROM operations WHERE state IN ('succeeded','failed','cancelled')`).all().map((r) => r.id);
    let prunedOld = 0;
    if (terminalIds.length) {
      prunedOld = db.prepare(`DELETE FROM operation_events WHERE ts < ? AND operation_id IN (SELECT id FROM operations WHERE state IN ('succeeded','failed','cancelled'))`).run(floor).changes;
    } else {
      prunedOld = 0;
    }
    // Per-operation cap: keep the newest perOpKeep rows per operation.
    const over = db.prepare(`
      DELETE FROM operation_events WHERE id IN (
        SELECT id FROM (
          SELECT id, ROW_NUMBER() OVER (PARTITION BY operation_id ORDER BY ts DESC, id DESC) AS rn
          FROM operation_events
        ) WHERE rn > ?
      )
    `).run(perOpKeep).changes;
    // Orphan events whose operation row is gone (defensive; FK cascade covers it).
    const orphans = db.prepare(`DELETE FROM operation_events WHERE operation_id NOT IN (SELECT id FROM operations)`).run().changes;
    return { prunedOld, capped: over, orphans };
  })();
}

/*
 * Phase 2B: destructive POSTs require Idempotency-Key or answer with the
 * existing operation. Returns { key } or sends the 400 and returns null.
 */
function requireIdempotencyKey(req, res) {
  const key = String((req.get && req.get('Idempotency-Key')) || '').trim();
  if (!key) {
...
module.exports = {
  ...
  isDestructiveKind,
  genId,
  pruneOperationEvents,
  requireIdempotencyKey,
  OPERATION_EVENTS_PER_OP_KEEP,
  OPERATION_EVENTS_RETENTION_MS,
};
