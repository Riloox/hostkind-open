...
const { redactObject, redactString } = require('./redact.cjs');
...
function createRelayClient(deps = {}) {
  ...
  async function submit(report) {
    ...
    try {
      ...
      const detail = redactString(String((err && err.message) || err || '')).text;
      const wrapped = new Error(
        timedOut
          ? 'bug-report relay: request timed out'
          : `bug-report relay unreachable: ${detail}`
      );
...
async function syncReportToRelay(report, deps = {}) {
  ...
  try {
    ...
    const error = redactString(String((err && err.message) || err || 'relay sync failed')).text;
