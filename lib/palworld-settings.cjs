...
const { PREVIEW_TTL_MS_SHORT, SNAPSHOT_RETENTION_SETTINGS } = require('../config/constants.cjs');
...
const previews = createPreviewStore({ ttlMs: PREVIEW_TTL_MS_SHORT });
...
function preview(server, actorId, body) {
  ...
  previews.set(token, {
    ...
    expiresAt: Date.now() + PREVIEW_TTL_MS_SHORT,
  });
  return {
...
function apply(server, actorId, body, idempotencyKey) {
  ...
  const snapshot = snapshots.take({
    ...
    kind: 'palworld-settings', reason: `Settings revision ${token.revision.slice(0, 12)}`, retention: SNAPSHOT_RETENTION_SETTINGS,
  });
...
function restore(server, id) {
  ...
  const rollback = snapshots.take({
    serverId: server.id, sourceDir: server.dir, scope: [relative],
    kind: 'palworld-settings', reason: `Before restoring ${id}`, retention: SNAPSHOT_RETENTION_SETTINGS,
  });
