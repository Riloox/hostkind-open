...
// Phase 2B: single-flight scoped by caller (server/user) instead of one global
// slot, so two operators (or two servers) can each browse while the same scope
// still never stacks two OS dialogs. Scope key defaults to 'global' to keep
// the old single-dialog behavior for callers that pass nothing.
const inFlightByKey = new Map();

function scopeKeyFor(deps = {}) {
  const raw = deps.scopeKey || deps.userId || deps.serverId || '';
  return String(raw || 'global');
}

async function pickFolder(defaultPath = '', title = DEFAULT_TITLE, deps = {}) {
  const scopeKey = scopeKeyFor(deps);
  if (inFlightByKey.get(scopeKey)) {
  ...
  inFlightByKey.set(scopeKey, true);
  try {
    let r;
...
function __resetInFlight(scopeKey) {
  if (scopeKey) inFlightByKey.delete(String(scopeKey));
  else inFlightByKey.clear();
}
