...
// tModLoader's Windows console input cannot be driven through Node's redirected
// stdin pipe. Its Mods directory also lives under the save root selected by
// `-tmlsavedirectory`; both the launch topology and that path are repaired for
// descriptors created before the explicit launch contract existed.
function tmodloaderLaunchArgs(args, saveDirectory) {
  const result = (Array.isArray(args) ? args : [])
    .filter((arg) => String(arg).toLowerCase() !== '-showserverconsole');
  const hasSaveDirectory = result.some((arg) => String(arg).toLowerCase() === '-tmlsavedirectory');
  const additions = [];
  if (!hasSaveDirectory && saveDirectory) additions.push('-tmlsavedirectory', saveDirectory);
  if (!additions.length) return result;
  const serverIndex = result.findIndex((arg) => String(arg).toLowerCase() === '-server');
  if (serverIndex >= 0) result.splice(serverIndex + 1, 0, ...additions);
  else result.push(...additions);
  return result;
}

function tmodloaderLaunch(executable, args, platform, systemRoot) {
  if (platform !== 'win32') return { executable, args };
  const isHeadlessConhost = path.basename(executable).toLowerCase() === 'conhost.exe'
    && String(args[0]).toLowerCase() === '--headless';
  if (isHeadlessConhost) return { executable, args };
  return {
    executable: path.join(systemRoot, 'System32', 'conhost.exe'),
    args: ['--headless', executable, ...args],
  };
}
...
function runPlayerAction(manager, action, target) {
  ...
  const result = { ok: true, source: 'console' };
...
function createTerrariaModule(deps = {}) {
  ...
  const windowsSystemRoot = String(deps.windowsSystemRoot || process.env.SystemRoot || process.env.WINDIR || 'C:\\Windows');
  ...
  return {
    ...
    start(manager) {
      ...
      let variant;
      try { variant = resolveVariant(desc); }
      ...
      const args = variant === 'tmodloader'
        ? tmodloaderLaunchArgs(desc.args, path.resolve(String(desc.dir || cwd)))
        : (Array.isArray(desc.args) ? [...desc.args] : []);
      ...
      const launch = variant === 'tmodloader'
        ? tmodloaderLaunch(executable, args, platform, windowsSystemRoot)
        : { executable, args };
      return manager._launch(launch.executable, launch.args, dotnet.env ? { env: dotnet.env } : {});
    },
    ...
    formatCommand(command, manager) {
      const variant = safeVariant(manager && typeof manager.desc === 'function' ? manager.desc() : null);
      const terminator = platform === 'win32' && variant === 'tmodloader' ? '\r\n' : '\n';
