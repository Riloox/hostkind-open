...
const { PREVIEW_TTL_MS_LONG, SNAPSHOT_RETENTION_DEFAULT } = require('../config/constants.cjs');

const FORMAT = 'fleetdeck-palworld-profile';
const PROFILE_VERSION = 1;
const SELECTIONS = Object.freeze(['configuration', 'world', 'mods', 'complete']);
const PREVIEW_TTL_MS = PREVIEW_TTL_MS_LONG;
...
const previews = createPreviewStore({
  ...
  onExpire: (token) => {
    fs.rmSync(path.join(importDir(), String(token)), { recursive: true, force: true });
  },
});
