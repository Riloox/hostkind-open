...
/*
 * Phase 2B retention, mirroring health.runRetention: idempotent prune of
 * crash intelligence older than the horizon. Groups cascade to incidents
 * (FK) and incidents cascade to conclusions, so deleting stale groups is
 * enough; orphan incidents/conclusions are swept defensively.
 */
const CRASH_RETENTION_MS = 90 * 24 * 3600 * 1000;

function pruneCrashes({ now = Date.now(), retentionMs = CRASH_RETENTION_MS } = {}) {
  const db = open();
  const floor = now - retentionMs;
  return db.transaction(() => {
    const groups = db.prepare('DELETE FROM crash_groups WHERE last_seen_at < ?').run(floor).changes;
    const orphanIncidents = db.prepare('DELETE FROM crash_incidents WHERE group_id NOT IN (SELECT id FROM crash_groups)').run().changes;
    const orphanConclusions = db.prepare('DELETE FROM crash_conclusions WHERE incident_id NOT IN (SELECT id FROM crash_incidents)').run().changes;
    return { groups, orphanIncidents, orphanConclusions };
  })();
}
...
function deleteServerData(serverId) {
  const db = open();
  db.transaction(() => {
    const groups = db.prepare('SELECT id FROM crash_groups WHERE server_id = ?').all(serverId).map((r) => r.id);
    db.prepare('DELETE FROM crash_incidents WHERE server_id = ?').run(serverId);
    db.prepare('DELETE FROM crash_groups WHERE server_id = ?').run(serverId);
    if (groups.length) {
      const remaining = db.prepare('SELECT id FROM crash_incidents WHERE group_id IN (SELECT id FROM crash_groups)').all();
      void remaining;
    }
    db.prepare(`DELETE FROM crash_conclusions WHERE incident_id NOT IN (SELECT id FROM crash_incidents)`).run();
  })();
}

module.exports = { capture, list, detail, acknowledge, pruneCrashes, deleteServerData, CRASH_RETENTION_MS, normalize, fingerprint, classify, safeTail, newestMatching, redact, DEFAULT_RULES, constants: { MAX_CONSOLE_LINES, MAX_FILE_BYTES, MAX_EVIDENCE_CHARS } };
