...
const MIGRATIONS = [
  ...
  {
    version: 16,
    name: 'multi-source-minecraft-content',
    up(db) {
      const addColumns = (table, additions) => {
        const columns = new Set(db.prepare(`PRAGMA table_info(${table})`).all().map((row) => row.name));
        for (const [name, type] of additions) if (!columns.has(name)) db.exec(`ALTER TABLE ${table} ADD COLUMN ${name} ${type}`);
      };
      addColumns('modpack_manifests', [
        ['display_name', 'TEXT'], ['version_name', 'TEXT'], ['provider_metadata_json', "TEXT NOT NULL DEFAULT '{}'"],
        ['source_kind', "TEXT NOT NULL DEFAULT 'catalog'"], ['verification_status', "TEXT NOT NULL DEFAULT 'verified'"],
      ]);
      addColumns('content_provenance', [
        ['display_name', 'TEXT'], ['version_name', 'TEXT'], ['provider_metadata_json', "TEXT NOT NULL DEFAULT '{}'"],
        ['source_kind', "TEXT NOT NULL DEFAULT 'catalog'"], ['verification_status', "TEXT NOT NULL DEFAULT 'verified'"],
      ]);
      db.exec(`
        UPDATE modpack_manifests SET provider = 'modrinth' WHERE provider IS NULL OR provider = '';
        UPDATE content_provenance SET provider = 'modrinth' WHERE provider IS NULL OR provider = '';
        CREATE INDEX IF NOT EXISTS content_provenance_provider_idx ON content_provenance(server_id, provider);
        CREATE INDEX IF NOT EXISTS modpack_manifests_provider_idx ON modpack_manifests(server_id, provider, installed_at DESC);
      `);
    },
  },
  {
    version: 17,
    name: 'phase-2b-indexes-cascade-retention',
    up(db) {
      // Phase 2B: missing query-pattern indexes, trigger-based cascades
      // (SQLite cannot ALTER an existing FK to add ON DELETE CASCADE, so
      // triggers emulate it idempotently), and helper tables for persistent
      // login throttling. Never edits older migrations; all statements are
      // IF NOT EXISTS so re-runs are no-ops.
      db.exec(`
        CREATE INDEX IF NOT EXISTS compatibility_cache_expiry_idx ON compatibility_cache(expires_at);
        CREATE INDEX IF NOT EXISTS compatibility_cache_source_expiry_idx ON compatibility_cache(source, expires_at);
        CREATE INDEX IF NOT EXISTS modpack_conflict_decisions_op_idx ON modpack_conflict_decisions(operation_id);
        CREATE INDEX IF NOT EXISTS audit_retention_previews_expiry_idx ON audit_retention_previews(expires_at);
        CREATE INDEX IF NOT EXISTS audit_retention_previews_actor_idx ON audit_retention_previews(actor_id, expires_at);
        CREATE INDEX IF NOT EXISTS audit_retention_requests_created_idx ON audit_retention_requests(created_at);
        CREATE INDEX IF NOT EXISTS health_baselines_computed_idx ON health_baselines(computed_at);
        CREATE INDEX IF NOT EXISTS health_baselines_server_metric_idx ON health_baselines(server_id, metric, computed_at DESC);
        CREATE INDEX IF NOT EXISTS health_analysis_computed_idx ON health_analysis(computed_at);
        CREATE INDEX IF NOT EXISTS health_settings_updated_idx ON health_settings(updated_at);
        CREATE INDEX IF NOT EXISTS world_inventory_server_idx ON world_inventory(server_id, scanned_at DESC);
        CREATE INDEX IF NOT EXISTS world_inventory_path_idx ON world_inventory(server_id, relative_path);
        CREATE INDEX IF NOT EXISTS template_import_previews_expiry_idx ON template_import_previews(expires_at);
        CREATE INDEX IF NOT EXISTS template_import_previews_actor_idx ON template_import_previews(actor_id, expires_at);
        CREATE INDEX IF NOT EXISTS crash_groups_server_idx ON crash_groups(server_id, last_seen_at DESC);
        CREATE INDEX IF NOT EXISTS crash_conclusions_incident_idx ON crash_conclusions(incident_id);
        CREATE INDEX IF NOT EXISTS crash_incidents_server_time_idx ON crash_incidents(server_id, occurred_at DESC);
        CREATE INDEX IF NOT EXISTS operation_events_ts_idx ON operation_events(ts);
        CREATE INDEX IF NOT EXISTS product_events_type_idx ON product_events(type, occurred_at DESC);
        CREATE INDEX IF NOT EXISTS product_events_server_idx ON product_events(server_id, occurred_at DESC);
      ...
      `);
      // Trigger-based cascades for FKs declared without ON DELETE CASCADE.
      // Templates own their versions; deleting a template drops its versions.
      db.exec(`
        CREATE TRIGGER IF NOT EXISTS templates_delete_cascade
        AFTER DELETE ON templates
        BEGIN
          DELETE FROM template_versions WHERE template_id = OLD.id;
        END;
      `);
      // Self-FK modpack_manifests.previous_manifest_id has no action: detach
      // children instead of failing the parent delete.
      db.exec(`
        CREATE TRIGGER IF NOT EXISTS modpack_manifests_self_detach
        BEFORE DELETE ON modpack_manifests
        BEGIN
          UPDATE modpack_manifests SET previous_manifest_id = NULL WHERE previous_manifest_id = OLD.id;
        END;
      `);
      // Server-scoped rows key off config.json servers (no DB parent table),
      // so cascade means "deleteServerData covers them". Triggers cannot fire
      // there; the retention/cleanup module owns that fan-out instead. This
      // migration only guarantees the indexes above plus the template/self-FK
      // triggers, which are the ones with real DB parents.
    },
  },
];
