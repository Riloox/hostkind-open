...
function open() {
  ...
  // Phase 2B: bound WAL growth under sustained writes. 1000 pages (~4MB with
  // the default page size) checkpoints often enough to keep the -wal file
  // small without the fsync storm of a tiny threshold.
  try { handle.pragma('auto_checkpoint = 1000'); } catch { /* older sqlite: ignore */ }
...
/*
 * Phase 2B: passive WAL checkpoint for the periodic maintenance tick.
 * PASSIVE never blocks writers; returns the checkpoint stats. Safe to call
 * on every interval alongside retention pruning.
 */
function checkpointPassive() {
  try {
    const db = open();
...
module.exports = { open, close, dbPath, dataDir, DATA_DIR, DB_PATH, checkpointPassive };
