  ...
  const inspection = {
    ...
    launchPlan: executable ? {
      ...
      args: selected === 'tmodloader'
        ? [path.join(root, 'tModLoader.dll'), '-server', '-tmlsavedirectory', root, '-config', configFile]
...
function adopt({ token, actorId, name, servers = [], variant = null, fixes = [] }) {
  ...
  try {
    if (selectedFixes.has('writeConfig') || selectedFixes.has('selectWorld')) {
      ...
      if (scope.length) {
        const snapshot = snapshots.take({ serverId: `terraria-adopt-${crypto.randomUUID()}`, sourceDir: root, scope, kind: 'terraria-adoption', reason: 'Before Terraria adoption fix', retention: SNAPSHOT_RETENTION_DEFAULT });
