...
function buildLaunchPlan(variant, root, configFile, host) {
  ...
  if (variant === 'tmodloader') {
    ...
    plan = {
      ...
      // tModLoader reads Mods from the save root selected by
      // `-tmlsavedirectory`, not Terraria's serverconfig `worldpath`. Keep that
      // root inside the installed tree; install/update promotion rewrites this
      // staging path to the final path. Windows console wrapping is applied by
      // the Terraria module at spawn time so the descriptor keeps its runtime.
      args: [resolved.entry, '-server', '-tmlsavedirectory', root, '-config', configFile],
