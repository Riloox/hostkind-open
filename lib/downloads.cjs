...
async function fetchToFile(url, destPath, opts = {}) {
  ...
  let done = 0;
  if (resume) {
    // Phase 2B: no existsSync->statSync TOCTOU. statSync is the check; ENOENT
    // means no resume state. A directory at partPath is refused outright.
    try {
      const st = fs.statSync(partPath);
      if (!st.isFile()) throw new DownloadError(`resume path is not a file: ${partPath}`, 'resume_conflict');
      done = st.size;
    } catch (err) {
      if (err && err.code === 'ENOENT') done = 0;
      else if (err instanceof DownloadError) throw err;
      else done = 0;
    }
  }
