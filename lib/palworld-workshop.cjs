...
const previews = createPreviewStore({ ttlMs: PREVIEW_TTL_MS });
...
function writeInventory(server, inventory) {
  ...
  atomicWriteFileSync(inventoryPath(server), JSON.stringify({
...
function saveSources(server, input) {
  ...
  const file = path.join(stateDir(server), 'sources.json');
  atomicWriteFileSync(file, JSON.stringify({ version: 1, manualPaths }, null, 2));
...
function updateSettingsAndInventory(server, packages) {
  ...
  atomicWriteFileSync(settings.file, settings.content);
  writeInventory(server, { packages });
}

async function downloadBatch({ server, workshopIds, cacheDir, download, ensureSteamCmdImpl = ensureSteamCmd, runSteamImpl = runSteamCmd, cachedPackagesImpl = cachedPackages }) {
  if (!Array.isArray(workshopIds)) fail('workshopIds must be an array.', 400, 'invalid_workshop_ids');
  const normalized = workshopIds.map((id) => String(id ?? '').trim());
  if (normalized.some((id) => !/^\d+$/.test(id))) fail('Workshop IDs must contain digits only.', 400, 'invalid_workshop_ids');
  const ids = [...new Set(normalized)];
  if (!ids.length) fail('Select at least one Workshop item.', 400, 'empty_selection');
  if (ids.length > 50) fail('Select no more than 50 Workshop items at once.', 400, 'selection_too_large');
  if (!cacheDir) fail('A SteamCMD cache directory is required.', 500, 'cache_dir_missing');
  const binary = await ensureSteamCmdImpl(cacheDir, download);
  const args = server?.dir ? ['+force_install_dir', server.dir, '+login', 'anonymous'] : ['+login', 'anonymous'];
  for (const id of ids) args.push('+workshop_download_item', APP_ID, id, 'validate');
  args.push('+quit');
  await runSteamImpl(binary, args, { cwd: cacheDir });
  const cached = cachedPackagesImpl(server);
  const downloaded = ids.filter((id) => cached.some((item) => item.workshopId === id));
  return {
    ok: downloaded.length === ids.length,
    requested: ids,
    downloaded,
    failed: ids.filter((id) => !downloaded.includes(id)),
  };
}

function setEnabledBatch({ server, manager, workshopIds, enabled }) {
  ...
  if (!ids.length) fail('Select at least one installed package.', 400, 'empty_selection');
  if (ids.length > 50) fail('Select no more than 50 installed packages at once.', 400, 'selection_too_large');
  ...
  const packages = ids.map((id) => {
    const pkg = inventory.packages.find((item) => item.workshopId === id);
    if (!pkg) fail('That official package is not installed.', 404, 'package_not_found');
    return pkg;
  });
  const changed = packages.filter((pkg) => pkg.enabled !== enabled);
  for (const pkg of changed) {
    pkg.enabled = enabled;
    pkg.updatedAt = new Date().toISOString();
  }
  if (changed.length) updateSettingsAndInventory(server, inventory.packages);
  return { ok: true, changed: changed.map((pkg) => pkg.workshopId), packages: inventory.packages, restartRequired: changed.length > 0 };
}
...
function remove({ server, manager, workshopId }) {
  ...
  const snapshot = snapshots.take({
    ...
    scope: ['Mods'],
    kind: 'palworld-workshop', reason: `Before removal of ${pkg.packageName}`, retention: SNAPSHOT_RETENTION_DEFAULT,
  });
...
function restore({ server, manager, trashId }) {
  ...
  catch { fail('That removed package is unavailable.', 404, 'trash_not_found'); }
  const inventory = readInventory(server);
  if (!inventory.readable) fail('The official mod inventory cannot be read.', 409, 'inventory_unreadable');
...
module.exports = {
  ...
  downloadBatch,
  setEnabled, setEnabledBatch, remove, listTrash, restore,
